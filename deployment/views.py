...
def whatsapp_oauth(request):
    ...
    try:
        ...
        if not chatbot_id:
            return Response(
                {"error": "chatbot_id is required"}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    try:
        ...
        try:
            ...
            whatsapp_business_url = (
                "https://graph.facebook.com/v24.0/me/whatsapp_business_account"
            )
            ...
            whatsapp_business_id = (
                whatsapp_data.get("data", [{}])[0].get("id")
                if "data" in whatsapp_data
                else None
            )
            ...
            if not whatsapp_business_id:
                return Response(
                    {
                        "error": "Failed to retrieve WhatsApp Business Account ID",
                        "details": whatsapp_data,
                    },
                    status=status.HTTP_400_BAD_REQUEST,
                )
    ...
    try:
        ...
        try:
                ...
                return Response(
                    {
                        ...
                        "details": accounts_data,
                    },
                    status=status.HTTP_400_BAD_REQUEST,
                )

            # Use the first Page (you can modify this to let the user choose a Page)
            page = accounts_data["data"][
                0
            ]  # e.g., 'Tees Heaven' with ID '119744001222118'
            page_id = page["id"]
            ...
            messenger_profile_url = (
                f"https://graph.facebook.com/v24.0/{page_id}/messenger_profile"
            )
            ...
            chatbot = Chatbot.objects.get(id=chatbot_id)
            messenger_api_url = f"https://graph.facebook.com/v24.0/{page_id}/messages"  # The API endpoint for sending messages
            ...
            webhook_setup_url = (
                f"https://graph.facebook.com/v24.0/{meta_client_id}/subscriptions"
            )
            webhook_params = {
                "object": "page",
                ...
                "verify_token": str(
                    chatbot.verify_token
                ),  # Use the chatbot's verify_token
            ...
            if "success" not in webhook_data or not webhook_data["success"]:
                return Response(
                    {
                        "error": "Failed to set up webhook in Meta Developer App",
            ...
            if "success" not in subscribe_data or not subscribe_data["success"]:
                return Response(
                    {
                        "error": "Failed to subscribe to Messenger webhooks for the Page",
            ...
            return Response(
                {
                    "success": True,
                    "message": "Messenger integration and webhook setup fully automated",
                },
                status=status.HTTP_200_OK,
            )
...
def instagram_oauth(request):
    ...
    try:
        ...
        # Return the authorization URL as a JSON response
        return Response({"authorization_url": full_auth_url}, status=status.HTTP_200_OK)
...
def instagram_oauth_callback(request):
    ...
    try:
        ...
        try:
            ...
            ig_business_url = f"https://graph.facebook.com/v24.0/{page_id}?fields=instagram_business_account"
            ...
            if (
                "instagram_business_account" not in ig_data
                or not ig_data["instagram_business_account"]
            ):
                return Response(
                    {
            ...
            chatbot = Chatbot.objects.get(id=chatbot_id)
            instagram_api_url = f"https://graph.facebook.com/v24.0/{ig_business_account_id}/messages"  # The API endpoint for sending messages
            chatbot.instagram_token = (
                page_access_token  # Store the Instagram Access Token
            )
            chatbot.instagram_page_id = (
                ig_business_account_id  # Store the Instagram Business Account ID
            )
            ...
            subscribe_url = f"https://graph.facebook.com/v24.0/{ig_business_account_id}/subscribed_apps"
            subscribe_params = {
                "subscribed_fields": "messages,messaging_postbacks",  # Events to subscribe to
            ...
            if "success" not in subscribe_data or not subscribe_data["success"]:
                print(
                    "Warning: Failed to subscribe to Instagram webhooks. You may need to complete App Review or manually subscribe in the Meta Developer Portal."
                )
                return Response(
                    {
                        "success": True,
            ...
            return Response(
                {
                    "success": True,
                    "message": "Instagram integration and webhook setup fully automated",
                },
                status=status.HTTP_200_OK,
            )
...
def whatsapp_credentials(request):
    try:
        ...
        webhook_url = f"{base_webhook_url}{chatbot.id}/whatsapp/"
        return JsonResponse(
            {"webhookurl": webhook_url, "verify_token": chatbot.verify_token},
            status=200,
        )
...
def messenger_credentials(request):
    try:
        ...
        # Return dynamic webhook URL and verify token
        webhook_url = f"{base_webhook_url}{chatbot.id}/messenger/"
        return JsonResponse(
            {"webhookurl": webhook_url, "verify_token": chatbot.verify_token},
            status=200,
        )
...
def instagram_credentials(request):
    try:
        ...
        # Return dynamic webhook URL and verify token
        webhook_url = f"{base_webhook_url}{chatbot.id}/instagram/"
        return JsonResponse(
            {"webhookurl": webhook_url, "verify_token": chatbot.verify_token},
            status=200,
        )
    ...
    if request.method == "GET":
        ...
        try:
            ...
            if platform == "whatsapp":
                ...
                if not entries:
                    return JsonResponse(
                        {"status": "success", "message": "No entries"}, status=200
                    )

                changes = entries[0].get("changes", [])
                if not changes:
                    return JsonResponse(
                        {"status": "success", "message": "No changes"}, status=200
                    )
                ...
                if not messages:
                    return JsonResponse(
                        {"status": "success", "message": "No messages"}, status=200
                    )
                ...
                if sender_wa_id and message_body:
                    ...
                    response = sendwhatsapp_messages(
                        sender_wa_id, message_body, whatsapp_id
                    )
                    return JsonResponse(
                        {
                            "status": "success",
                ...
                for entry in data.get("entry", []):
                    ...
                    for messaging_event in entry.get("messaging", []):
                        if "message" in messaging_event:
                            ...
                            response = messenger_messages(
                                sender_id, message_text, messenger_id
                            )
                            return JsonResponse(
                                {
                                    "status": "success",
                ...
                for entry in data.get("entry", []):
                    ...
                    for messaging_event in entry.get("messaging", []):
                        if "message" in messaging_event:
                            sender_id = messaging_event["sender"]["id"]
                            message_text = messaging_event["message"]["text"]
                            response = instagram_messages(
                                sender_id, message_text, instagram_id
                            )
                            return JsonResponse(
                                {
                                    "status": "success",
            ...
            return JsonResponse(
                {"status": "success", "message": "No actionable data"}, status=200
            )
